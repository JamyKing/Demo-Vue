<template>
  <div>
    <el-row class="row-style">
      ...
      <el-button
        @click="wordChange('chinese')"
        :disabled="typingStatus"
        type="primary"
        size="small"
        :plain="wordType !== 'chinese'"
        >中文</el-button
      >
      <el-button
        @click="wordChange('english')"
        :disabled="typingStatus"
        type="primary"
        size="small"
        :plain="wordType !== 'english'"
        >英文</el-button
      >
      <el-button
        @click="wordChange('number')"
        :disabled="typingStatus"
        type="primary"
        size="small"
        :plain="wordType !== 'number'"
        >数字</el-button
      >
      <el-input-number
        @change="timeChoose"
    ...
    <el-row class="row-style">
      <el-col :span="10">
        <el-button @click="begin" type="success" size="small" :disabled="start"
          >开始</el-button
        >
        <el-button
          @click="suspend"
      ...
      <el-col :span="14">
        <label class="timeData"
          >时间：{{ time.hour }}:{{ time.minute }}:{{ time.second }}</label
        >
        ...
        <div class="wordpad" v-for="(items, indexs) in wordSplit" :key="indexs">
          <div class="refer">
            <span
              ...
              :class="{
                red:
                  item !== WatchWord[indexs][index] &&
                  typeof WatchWord[indexs][index] !== 'undefined',
                green: item === WatchWord[indexs][index],
              }"
            >
              {{ item === " " ? "&nbsp;" : item }}
            </span>
          ...
          <div ref="enterInput" class="enter">
            ...
            <div class="PositionWord">
              <span
                v-for="(item, index) in WatchWord[indexs]"
                :key="index"
                :class="{ red: item !== typingList[index] }"
                >{{ item }}</span
              >
            </div>
          </div>
    ...
    >
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="customSubmit" size="medium"
          >导 入</el-button
        >
      </span>
    </el-dialog>
    ...
    >
      <div class="result-div">
        <div class="result-data u-f-cer">
          <label
            >消耗时间：{{ timeTemp.hour }}:{{ timeTemp.minute }}:{{
              timeTemp.second
            }}</label
          >
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="resultClose" size="medium"
          >确 定</el-button
        >
      </span>
    </el-dialog>
      ...
      chineses: [
        {
          ...
          type: "chinese",
          value:
            "盼望着，盼望着，东风来了，春天的脚步近了。一切都像刚睡醒的样子，欣欣然张开了眼。山朗润起来了，水涨起来了，太阳的脸红起来了。小草偷偷地从土里钻出来，嫩嫩的，绿绿的。园子里，田野里，瞧去，一大片一大片满是的。坐着，躺着，打两个滚，踢几脚球，赛几趟跑，捉几回迷藏。风轻悄悄的，草软绵绵的。桃树、杏树、梨树，你不让我，我不让你，都开满了花赶趟儿。红的像火，粉的像霞，白的像雪。花里带着甜味儿；闭了眼，树上仿佛已经满是桃儿、杏儿、梨儿。花下成千成百的蜜蜂嗡嗡地闹着，大小的蝴蝶飞来飞去。野花遍地是：杂样儿，有名字的，没名字的，散在草丛里，像眼睛，像星星，还眨呀眨的。",
        },
        {
          label: "勇敢的人生",
          type: "chinese",
          value:
            "在工作中，你是否有过这样的感受：你平时尽职尽责，任劳任怨，就因为无意做错了一件小事，就被领导全盘否定。你待人宽厚，善良，不计较，但就因为无心说错了一句话，就被同事怀恨在心。甚至你加班熬夜，好不容易做出来的方案，却因为客户的过度挑剔，而毁于一旦。你时常觉得上班很累，不仅因为压力大，任务重，担子沉，还因为这里勾心斗角，尔虞我诈，明争暗斗等等，让你感到力不从心。你曾以为，就自己很苦，很忙，很疲惫，但你不知道的是，这个世界，还有更多的人，过得比你还辛苦，但都在负重前行，咬牙坚持。你曾以为，换一个地方就好了，换一个环境就对了，换一批人就轻松了。但你不知道的是，其实到哪儿都要面对该有的困难和挫折。",
        },
      ],
      englishs: [
        {
      ...
      numbers: [
        {
          ...
          type: "number",
          value:
            "18305.230184.6835.1730.4824.71564170.6310.62063951279.468246.96437.5041.2795.48517037.1461960.3790.5280.4519.6168405286.3961048375.7937456.2749537059635.638960.3743854072.48942706.396758.46249247.275370641970.4683964817.48619369.3957260.59.28.40.17.59274061286",
        },
      ],
    };
  },
      ...
      return {
        schedule:
          this.currentWord.length <= 0
            ...
            : (
                Math.round(
                  (this.enterSplit.join("").length / this.currentWord.length) *
                    10000
                ) / 100.0
              ).toFixed(2) + "%",
      };
  ...
  watch: {
    WatchWord: {
      handler(newValue, oldValue) {
        ...
        newValue.forEach((i, c, v) => {
          if (i instanceof Array && i) {
            if (i.length >= this.wordSplit[c].length) {
              ...
              this.enterSplit[c] = this.enterSplit[c].slice(
                0,
                this.wordSplit[c].length
              );
            }
            i.forEach((ip, cp, vp) => {
        ...
        // 准确率计算
        this.accuracy =
          this.enterSplit.join("").length - proper.length <= 0
            ? "0.00"
            : (
                ((this.enterSplit.join("").length - proper.length) * 100) /
                this.enterSplit.join("").length
              )
                .toFixed(2)
                .toString();
        if (
          this.enterSplit.join("").length >= this.currentWord.split("").length
        ) {
          //不去空格
          //当写入完成结束时
          this.finish();
  ...
  methods: {
    init() {
      ...
      if (this.typingList.length > this.splitSize) {
        var hang =
          this.typingList.length % this.splitSize == 0
            ? parseInt(this.typingList.length / this.splitSize)
            : parseInt(this.typingList.length / this.splitSize) + 1;
    ...
    finish() {
      ...
      if (type === "default") {
        ...
        if (timeSum >= 60) {
          ...
          this.timeTemp.minute =
            minuteTemp < 10 ? "0" + minuteTemp : minuteTemp;
          timeSum = timeSum - minuteTemp * 60;
        }
        if (timeSum > 0) {
          let secondTemp = timeSum;
          this.timeTemp.second =
            secondTemp < 10 ? "0" + secondTemp : secondTemp;
        }
      }
      let enterLength = enterSplit.join("").length;
      this.speed =
        enterLength > 0 ? Math.round((enterLength / consume) * 60) : 0;
